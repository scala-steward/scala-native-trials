...
object VirtualRef {
  def unapply(addr: Addr)(implicit
      state: State): Option[(Kind, Class, Array[Val])] =
    unapply(Val.Virtual(addr))
  def unapply(value: Val)(implicit
      state: State): Option[(Kind, Class, Array[Val])] = value match {
    case Val.Virtual(addr) =>
...
object CompRef {
  def unapply(addr: Addr)(implicit
      state: State): Option[(Comp, Type, Val, Val)] =
    unapply(Val.Virtual(addr))
  def unapply(value: Val)(implicit
      state: State): Option[(Comp, Type, Val, Val)] =
    value match {
